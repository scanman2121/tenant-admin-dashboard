...
export default function InvitedEmployees() {
    const [isLoading, setIsLoading] = useState(true)
    const [error, setError] = useState<string | null>(null)
    const [invitedEmployeeData, setInvitedEmployeeData] = useState(invitedUsers)
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                setInvitedEmployeeData(invitedUsers)
                setError(null)
            } catch (err) {
                console.error("Error loading invited employee data:", err)
                setError("Failed to load invited employee data. Please refresh the page.")
                showError("Failed to load invited employee data. Please refresh the page.")
            } finally {
                setIsLoading(false)
            }
        }
                ...
                <div className="flex items-center justify-between bg-gray-50 px-4 py-3.5 dark:bg-gray-900">
                    <div className="flex items-center gap-2">
                        <span className="text-xs font-medium uppercase text-gray-500">
                            {invitedEmployeeData.length} invited
                        </span>
                    </div>
                </div>
                <div className="p-0">
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-800 dark:bg-gray-950">
                            {invitedEmployeeData.map((employee) => (
                                <tr key={employee.email} className="hover:bg-gray-50 hover:dark:bg-gray-800">
